...
from vericode_app.Serializer.Serializer import ApiRequestSerializer, LlmModelSerializer, LlmModelsSerializer,SummarizeLlmModelsSerializer
...
@api_view(['GET'])# Create your views here.
def getSummarizeLlmModels(request):
    if request.method == 'GET':
        summarizeLlmModels = SummarizeLlmModels.objects.all()
        serializer = SummarizeLlmModelsSerializer(summarizeLlmModels, many=True)
        return Response(serializer.data,status=status.HTTP_200_OK)
...
class GetMergeModelsView(APIView):
    def __init__(self, api_service=None,model_mapper=None, *args, **kwargs):
        self.api_service = api_service or ApiService()
        self.model_mapper = model_mapper or ModelMapper()
        super().__init__(*args, **kwargs)
